...
#define NUC126_PWM_CLKSRC_HSE  0x0UL
#define NUC126_PWM_CLKSRC_HCLK 0x2UL
#define NUC126_PWM_CLKSRC_HSI  0x3UL
#define NUC126_PWM_CLKSRC_LSI  0x7UL
 ...
 */
#if (NUC126_PWM_USE_PWM1 == TRUE) || defined(__DOXYGEN__)
PWMDriver PWMD1;
#endif
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(NUC126_PWMA_HANDLER)
{
  OSAL_IRQ_PROLOGUE();
...
void pwm_lld_start(PWMDriver *pwmp)
{
  ...
  if (pwmp->state == PWM_STOP) {
    ...
    clksel1 = (clksel1 & ~(CLK_CLKSEL1_PWM01_S_Msk)) |
              ((NUC126_PWM_CLKSRC_HCLK << CLK_CLKSEL1_PWM01_S_Pos) &
               CLK_CLKSEL1_PWM01_S_Msk);
    clksel1 = (clksel1 & ~(CLK_CLKSEL1_PWM23_S_Msk)) |
              ((NUC126_PWM_CLKSRC_HCLK << CLK_CLKSEL1_PWM23_S_Pos) &
               CLK_CLKSEL1_PWM23_S_Msk);
    clksel2 = (clksel2 & ~(CLK_CLKSEL2_PWM01_S_E_Msk)) |
              ((NUC126_PWM_CLKSRC_HCLK >> 2) << CLK_CLKSEL2_PWM01_S_E_Pos);
    clksel2 = (clksel2 & ~(CLK_CLKSEL2_PWM23_S_E_Msk)) |
              ((NUC126_PWM_CLKSRC_HCLK >> 2) << CLK_CLKSEL2_PWM23_S_E_Pos);
    CLK->CLKSEL1 = clksel1;
    CLK->CLKSEL2 = clksel2;

    CLK->APBCLK |= (CLK_APBCLK_PWM01_EN_Msk | CLK_APBCLK_PWM23_EN_Msk);
    nvicEnableVector(NUC126_PWMA_NUMBER, NUC126_PWM_IRQ_PRIORITY);
    ...
    prescale = NUC126_HCLK / pwmp->config->frequency;
    ...
    if (pwmp->config->channels[0].mode != PWM_OUTPUT_DISABLED) {
      if (pwmp->config->channels[0].pinmask & NUC126_PWM_CH0_PIN_PA12) {
        SYS->GPA_MFP |= (1 << 12);
      }
    }

    if (pwmp->config->channels[1].mode != PWM_OUTPUT_DISABLED) {
      if (pwmp->config->channels[1].pinmask & NUC126_PWM_CH1_PIN_PA13) {
        SYS->GPA_MFP |= (1 << 13);
      }
    }

    if (pwmp->config->channels[2].mode != PWM_OUTPUT_DISABLED) {
      if (pwmp->config->channels[2].pinmask & NUC126_PWM_CH2_PIN_PA14) {
        SYS->GPA_MFP |= (1 << 14);
      }

      if (pwmp->config->channels[2].pinmask & NUC126_PWM_CH2_PIN_PC12) {
        SYS->GPC_MFP |= (1 << 12);
    ...
    if (pwmp->config->channels[3].mode != PWM_OUTPUT_DISABLED) {
      if (pwmp->config->channels[3].pinmask & NUC126_PWM_CH3_PIN_PA15) {
        SYS->GPA_MFP |= (1 << 15);
        SYS->ALT_MFP &= ~SYS_ALT_MFP_PA15_MFP1_Msk;
      }

      if (pwmp->config->channels[3].pinmask & NUC126_PWM_CH3_PIN_PC13) {
        SYS->GPC_MFP |= (1 << 13);
...
void pwm_lld_stop(PWMDriver *pwmp)
{
  ...
  if (pwmp->state == PWM_READY) {
    ...
    if (&PWMD1 == pwmp) {
      CLK->APBCLK &= ~(CLK_APBCLK_PWM01_EN_Msk | CLK_APBCLK_PWM23_EN_Msk);
      nvicDisableVector(NUC126_PWMA_NUMBER);
      return;
    }
#endif
  }
