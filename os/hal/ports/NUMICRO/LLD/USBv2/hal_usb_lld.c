...
#define NUC126_USBD_BUSSTATUS_INACK       0UL
#define NUC126_USBD_BUSSTATUS_INNAK       1UL
#define NUC126_USBD_BUSSTATUS_OUTDATA0ACK 2UL
#define NUC126_USBD_BUSSTATUS_SETUPACK    3UL
#define NUC126_USBD_BUSSTATUS_OUTDATA1ACK 6UL
#define NUC126_USBD_BUSSTATUS_ISOEND      7UL
...
static void usb_lld_serve_interrupt(USBDriver* usbp)
{
  ...
  if (intsts & USBD_INTSTS_VBDETIF_Msk) {
    if (USBD->VBUSDET & USBD_VBUSDET_VBUSDET_Msk) {
      USBD->ATTR |= (USBD_ATTR_PHYEN_Msk | USBD_ATTR_USBEN_Msk);
      usbConnectBus(&USBD1);
    } else {
  ...
  if (intsts & USBD_INTSTS_WKIDLEIF_Msk) {
    /* Clear event flag */
    USBD->INTSTS = USBD_INTSTS_WKIDLEIF_Msk;
    _usb_wakeup(usbp);
  }
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(NUC126_USB1_HANDLER)
{
  OSAL_IRQ_PROLOGUE();
...
void usb_lld_init_endpoint(USBDriver* usbp, usbep_t ep)
{
  ...
  if (usbp->epc[ep]->out_state) {
    ...
    case USB_EP_MODE_TYPE_CTRL:
      HW_OUT_EP(ep)->CFG =
          ...
          NUC126_USBD_CFG_OUT | USBD_CFG_CSTALL_Msk;
      break;
    case USB_EP_MODE_TYPE_ISOC:
      HW_OUT_EP(ep)->CFG =
          ((ep << USBD_CFG_EPNUM_Pos) & USBD_CFG_EPNUM_Msk) |
          NUC126_USBD_CFG_OUT | USBD_CFG_ISOCH_Msk;
      break;
  ...
  if (usbp->epc[ep]->in_state) {
    ...
    case USB_EP_MODE_TYPE_CTRL:
      HW_IN_EP(ep)->CFG =
          ((ep << USBD_CFG_EPNUM_Pos) & USBD_CFG_EPNUM_Msk) |
          NUC126_USBD_CFG_IN | USBD_CFG_CSTALL_Msk;
      break;
    case USB_EP_MODE_TYPE_ISOC:
      HW_IN_EP(ep)->CFG =
          ((ep << USBD_CFG_EPNUM_Pos) & USBD_CFG_EPNUM_Msk) |
          NUC126_USBD_CFG_IN | USBD_CFG_ISOCH_Msk;
      break;
    case USB_EP_MODE_TYPE_BULK:
    case USB_EP_MODE_TYPE_INTR:
    default:
